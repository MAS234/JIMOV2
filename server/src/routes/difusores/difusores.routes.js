...
difusores.get("/listarDifusores", async (req, res) => {
  ...
  //Agregar Difusores
difusores.post("/agregarDifusor", upload.single('imagen'), async (req, res) => {
  try {
    ...
    const storageRef = ref(storage, `difusores/${uuidv4()}-${file.originalname}`);
    const metadata = {
    ...
    // Guardar detalles de la Difusores en Firestore
    const newDifusores = {
      descripcion,
      nombre,
      precio,
    ...
    const docRef = await addDoc(collection(db, 'difusores'), newDifusores);

    // Responder con éxito y el ID del nuevo documento
    res.json({ success: true, id: docRef.id, data: newDifusores });
  } catch (error) {
    console.error('Error adding Difusores:', error);
    res.status(500).json({ success: false, error: 'Error adding Difusores' });
  }
  });
    ...
    difusores.delete("/eliminarDifusor/:id", async (req, res) => {
      try {
        ...
        // Eliminar el documento de la difusores en Firestore
        ...
        res.json({ success: true, message: 'difusores eliminada correctamente' });
      } catch (error) {
        console.error('Error deleting difusores:', error);
        res.status(500).json({ success: false, error: 'Error eliminando difusores' });
      }
      });

//ACTUALIZAR DIFUSORES
difusores.put('/actualizarDifusor/:id', upload.single('imagen'), async (req, res) => {
  try {
    const { id } = req.params;
    const { descripcion, nombre, precio } = req.body;
    const file = req.file;
    ...
    // Responder con éxito
    res.json({ success: true, message: 'Difusor actualizada correctamente' });
  } catch (error) {
