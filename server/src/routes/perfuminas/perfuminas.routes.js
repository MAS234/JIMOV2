...
//Agregar Perfuminas
perfuminas.post("/agregarPerfumina", upload.single('imagen'), async (req, res) => {
  try {
    ...
    const storageRef = ref(storage, `perfuminas/${uuidv4()}-${file.originalname}`);
    const metadata = {
    ...
    // Guardar detalles de la perfuminas en Firestore
    const newperfuminas = {
      descripcion,
      nombre,
      precio,
    ...
    const docRef = await addDoc(collection(db, 'perfuminas'), newperfuminas);

    // Responder con éxito y el ID del nuevo documento
    res.json({ success: true, id: docRef.id, data: newperfuminas });
  } catch (error) {
    console.error('Error adding perfuminass:', error);
    res.status(500).json({ success: false, error: 'Error adding perfuminas' });
  }
  });
...
perfuminas.delete("/eliminarPerfumina/:id", async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Perfuminas eliminada correctamente' });
  } catch (error) {
    console.error('Error deleting Perfuminas:', error);
    res.status(500).json({ success: false, error: 'Error eliminando Perfuminas' });
  }
  });

  //ACTUALIZAR PERFUMINAS
perfuminas.put('/actualizarPerfumina/:id', upload.single('imagen'), async (req, res) => {
  try {
    const { id } = req.params;
    const { descripcion, nombre, precio } = req.body;
    const file = req.file;
    ...
    // Responder con éxito
    res.json({ success: true, message: 'Perfumina actualizada correctamente' });
  } catch (error) {
