...
concetrados.post("/agregarConcentrado", upload.single('imagen'), async (req, res) => {
  try {
    ...
    const storageRef = ref(storage, `concentradosPisos/${uuidv4()}-${file.originalname}`);
    const metadata = {
    ...
    // Guardar detalles de la Concentrados en Firestore
    ...
    res.json({ success: true, id: docRef.id, data: newConcentrado });
  } catch (error) {
  ...
  concetrados.delete("/eliminarConcentrado/:id", async (req, res) => {
    try {
      ...
      // Eliminar el documento de la concentrado en Firestore
      ...
      res.json({ success: true, message: 'concentrado eliminada correctamente' });
    } catch (error) {
...
concetrados.put('/actualizarConcentrado/:id', upload.single('imagen'), async (req, res) => {
  ...
  try {
    ...
    // Responder con éxito
    res.json({ success: true, message: 'CONCENTRADO actualizada correctamente' });
  } catch (error) {
