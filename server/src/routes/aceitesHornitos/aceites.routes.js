...
aceites.get("/listarAceites", async (req, res) => {
    try {
      ...
      //Agregar Aceites
aceites.post("/agregarAceites", async (req, res) => {
    try {
   ...
   aceites.delete("/eliminarAceite/:id", async (req, res) => {
    try {
      ...
      res.json({ success: true, message: "Aceite eliminado correctamente" });
    } catch (error) {
    ...
    //ACTUALIZAR ACEITES
aceites.put('/actualizarAceite/:id', async (req, res) => {
    try {
      const { id } = req.params;
      const { descripcion, nombre, precio } = req.body;
      ...
      // Responder con éxito
      res.json({ success: true, message: 'Aceite actualizado correctamente' });
    } catch (error) {
